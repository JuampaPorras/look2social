...
function closePopOver(thisElement) {
...
function getCommentsBySmartCharId(smartCharIdValue, successCallBack, errorCallback) {
    $.ajax({
        type: "GET",
        url: "../WS/CommentsWS.asmx/GetCommentsByChart",
        data: { smartChartId: smartCharIdValue },
        contentType: "application/json; charset=utf-8",
        dataType: "text", // the data type we want back, so text.  The data will come wrapped in xml
        success: function (data) {
            var comments = JSON.parse(data);
            if (comments.d.Acknowledgment) {
                successCallBack(comments.d);
            } else {
                errorCallback();
            }
...
function postComment(idSmartChart) {
    ...
    var message = $('.popover #txtComment_' + idSmartChart).val();
    if (message != "") {
        $.ajax({
            type: "POST",
            url: "../WS/CommentsWS.asmx/AddChartComment",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: JSON.stringify({ "IdUser": currentUserId, "IdSmartChart": idSmartChart, "Message": message }),
            success: function(data) {
                if (data.d.Acknowledgment) {
